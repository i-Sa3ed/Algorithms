...
int getMaxArea(vector<int> &heights, int l, int r) {
    ...
    int min_idx = getMinIdx(l, r);	// ready for you
    int result = heights[min_idx] * (r - l + 1); // either take this index with the whole range
    result = max({result, getMaxArea(heights, l, min_idx - 1), getMaxArea(heights, min_idx + 1, r)});
    return result;
}
